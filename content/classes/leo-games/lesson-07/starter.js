// The Brick Wall 🧱
// A wall is just an ARRAY of brick boxes. Today you build the grid, then knock
// out every brick the ball touches. Smash them all to WIN!
...
// ┌─────────────────────────────────────────────────────────────┐
// │  YOUR JOB: finish makeBricks AND removeHit below 👇          │
// └─────────────────────────────────────────────────────────────┘

// makeBricks builds the wall: a grid of brick boxes, rows DOWN and columns
// ACROSS. Start an empty array, then a loop INSIDE a loop pushes one box per
// spot. Each brick is w=50, h=16. Bricks sit gapX=6 apart sideways and gapY=6
// apart up-and-down; the wall starts at left=9, top=30.
//   brick x = left + c * (w + gapX)      (c = column number)
//   brick y = top  + r * (h + gapY)      (r = row number)
function makeBricks(rows, cols) {
  // const bricks = [];
  // const w = 50, h = 16, gapX = 6, gapY = 6, left = 9, top = 30;
  // for (let r = 0; r < rows; r++) {
  //   for (let c = 0; c < cols; c++) {
  //     bricks.push({ x: ..., y: ..., w: w, h: h });
  //   }
  // }
  // return bricks;
}

// removeHit returns a NEW array of bricks with every brick the box touched
// dropped out. Use .filter: KEEP a brick only when it does NOT overlap the box.

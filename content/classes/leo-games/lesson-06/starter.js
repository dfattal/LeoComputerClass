// Keeping Score 🏆
// The whole game is ONE bundle of numbers: the "state". It holds the ball, the
// paddle, the SCORE, and how many LIVES are left. Today you teach the game to
// score points, lose lives, and shout GAME OVER.
...
// ┌─────────────────────────────────────────────────────┐
// │  YOUR JOB: finish the three score-keeping functions 👇 │
// └─────────────────────────────────────────────────────┘
// The big rule: each one RETURNS A NEW state — it never changes the state you
// were handed. The magic word is the spread `...state`, which means "copy every
// key from state, then overwrite just the one I name."

// 1) addScore: return a new state with `points` added to the score.
function addScore(state, points) {
  // return { ...state, score: state.score + points };
}

// 2) loseLife: return a new state with one fewer life.
function loseLife(state) {
  // return { ...state, lives: state.lives - 1 };
}

// 3) isGameOver: return true when there are no lives left (0 or fewer).
